...
class CLdap
{
	...
	public function __construct($arg = [])
	{
		...
		if (is_array($arg)) {
			$this->cnf = $this->zbx_array_merge($this->cnf, $arg);
		}
	...
	public function connect()
	{
		...
		if ($this->cnf['version']) {
			if (!@ldap_set_option($this->ds, LDAP_OPT_PROTOCOL_VERSION, $this->cnf['version'])) {
				...
				// needs version 3
				if (
					!$this->zbx_empty($this->cnf['referrals'])
	...
	public function getUserData($user)
	{
		...
		$user_result = $this->zbx_array_merge($info, $user_result);
	...
	private function zbx_array_merge()
	{
		$args = func_get_args();
		$result = [];
	...
	private function zbx_empty($value)
	{
		if ($value === null) {
			return true;
		}
